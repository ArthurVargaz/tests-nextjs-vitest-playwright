...
test.describe('<Home /> (E2E)', () => {
  ...
  test.describe('Criação', () => {
    ...
    test('deve permitir que eu crie mais de 1 TODO', async ({ page }) => {
      const { btn, input } = getAll(page);
  ...
  test.describe('Exclusão', () => {
    ...
    test('deve permitir apagar todos TODOs', async ({ page }) => {
  ...
  test.describe('Errors', () => {
    test('deve mostrar erro se a descrição tem 3 ou menos caracteres', async ({
      ...
      const errorText = 'Descricao precisa ter mais de 3 caracteres';
    ...
    test('deve mostrar erro se um TODO já existir com a mesma descrição', async ({
      ...
      await input.fill('eu ja existo');
      await btn.click();
      await input.fill('eu ja existo');
      await btn.click();

      const errorText = 'Ja existe um todo com o ID ou descrição enviados';
    ...
    test('deve remover o erro da tela quando o usuário corrigir o erro', async ({
      ...
      await input.fill('Essa descrição é valida');
      await btn.click();
